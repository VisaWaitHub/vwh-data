...
def _find_value_on_or_before(history_dates, history_values, target_date):
    """
    Returns the value for the latest snapshot on or before target_date.
    ...
    for ds, vs in zip(history_dates, history_values):
        ...
        if d <= target_date:
            ...
            # since dates are ascending, once we pass target_date we can stop
            break
    return best
...
def _compute_post_derivatives(p):
    ...
    # Deltas: positive means wait increased; negative means improved
    p["delta_7d"] = (last_val - int(v7)) if v7 is not None else None
    p["delta_30d"] = (last_val - int(v30)) if v30 is not None else None

    # last_change_at: scan backwards for most recent day where value changed
    last_change = ""
    ...
    # trend_direction based on delta_7d if available, else delta_30d
